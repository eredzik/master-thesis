...
def node_link_data(G, attrs=_attrs):
    ...
    if multigraph:
        data["links"] = [
            dict(
                chain(d.items(), [(source, mapping[u]), (target, mapping[v]), (key, k)])
            )
            for u, v, k, d in G.edges_iter(keys=True, data=True)
        ]
    else:
        data["links"] = [
            dict(chain(d.items(), [(source, mapping[u]), (target, mapping[v])]))
...
def node_link_graph(data, directed=False, multigraph=True, attrs=_attrs):
    ...
    for d in data["links"]:
        ...
        if not multigraph:
            edgedata = dict(
                (make_str(k), v) for k, v in d.items() if k != source and k != target
            )
